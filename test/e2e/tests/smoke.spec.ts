...
test.describe('harness', () => {
  test('vanilla-fibonacci.html upgrades and renders into the shadow DOM', async ({ page }) => {
    ...
    await expect(wc.locator('css=mark[data-ann-id="a1"]').first()).toBeVisible();
    await expect(wc.locator('css=mark[data-ann-id="a2"]')).toBeVisible();
  });

  test('vanilla-multiple-blocks.html renders three independent instances', async ({ page }) => {
