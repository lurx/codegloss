...
test.describe('keyboard navigation', () => {
  ...
  test('Tab walks through the toolbar buttons in order', async ({ page, browserName }) => {
    ...
    test.skip(browserName === 'webkit', 'WebKit default tab order skips buttons');
  ...
  test('Enter on the focused copy button triggers the copy action', async ({
    ...
    await page.addInitScript(() => {
      ...
      try {
        ...
        (navigator as unknown as { clipboard: { writeText: typeof fake } }).clipboard = {
          writeText: fake,
        };
      }
    });
    ...
    await page.locator('code-gloss').evaluate((el) => {
      const btn = el.shadowRoot?.querySelector<HTMLButtonElement>('.copyButton');
      btn?.focus();
    });
