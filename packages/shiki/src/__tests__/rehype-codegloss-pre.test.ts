...
function pre(
	codeChildren: ElementContent[],
	codeProps: Element['properties'] = {},
): Element {
	return {
		type: 'element',
		tagName: 'pre',
...
function readConfig(element: Element): Record<string, unknown> {
	const script = element.children[0] as Element;
...
describe('rehypeCodeglossPre', () => {
	...
	it('serializes highlighter spans preserving class + kebab-cased data attrs', () => {
		...
		expect(config.highlightedHtml).toBe(
			'<span class="tok" data-lang="js">const</span> x;',
		);
	});
	...
	it('escapes text entities in serialized HTML', () => {
		...
		expect(readConfig(tree.children[0] as Element).highlightedHtml).toBe(
			'&lt;x&gt;&amp;"',
		);
	});
	...
	it('skips undefined / null / false attribute values', () => {
		...
		const html = readConfig(tree.children[0] as Element)
			.highlightedHtml as string;
	...
	it('escapes </script in the config JSON to prevent early termination', () => {
		...
		expect(scriptText.value).toContain(String.raw`<\/script`);
	});
