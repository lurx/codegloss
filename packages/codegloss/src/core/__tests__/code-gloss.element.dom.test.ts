...
import type { Annotation, CodeGlossConfig, Connection } from '../code-gloss.types';
...
function mount(
  ...
  if (opts.highlight) el.highlight = opts.highlight;

  if (opts.withScript !== false) {
    const script = document.createElement('script');
    script.setAttribute('type', 'application/json');
    script.textContent =
      opts.rawScriptText ?? JSON.stringify({ code: '', lang: 'js', ...config });
...
const shadow = (el: CodeGlossElement) => el.shadowRoot as ShadowRoot;
...
beforeAll(() => {
  ...
  (HTMLElement.prototype as { showPopover?: () => void }).showPopover =
    vi.fn();
...
describe('CodeGlossElement', () => {
  describe('connectedCallback / DOM construction', () => {
    it('builds the shadow DOM from a JSON script child', () => {
      const el = mount({ code: 'let x = 1', lang: 'js', filename: 'app.js' });
    ...
    it('renders one line element per line of source code', () => {
      const el = mount({ code: 'one\ntwo\nthree', lang: 'js' });
      expect(shadow(el).querySelectorAll('.line')).toHaveLength(3);
    });

    it('renders an error message when the JSON script child is missing', () => {
      const el = mount({}, { withScript: false });
      expect(shadow(el).innerHTML).toContain('missing or invalid config');
    });

    it('renders an error message and logs when the JSON is malformed', () => {
      const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
      const el = mount({}, { rawScriptText: '{not json' });
    ...
    it('renders an error message when the JSON script is empty', () => {
      const el = mount({}, { rawScriptText: '' });
      expect(shadow(el).innerHTML).toContain('missing or invalid config');
    });

    it('omits the filename node when no filename is configured', () => {
      const el = mount({ lang: 'js', code: 'x' });
      expect(shadow(el).querySelector('.filename')).toBeNull();
    });

    it('hides the run button for non-js languages by default', () => {
      const el = mount({ lang: 'py', code: 'print(1)' });
      expect(shadow(el).querySelector('.runButton')).toBeNull();
    });

    it('shows the run button for non-js languages when runnable is true', () => {
      const el = mount({ lang: 'py', code: 'print(1)', runnable: true });
      expect(shadow(el).querySelector('.runButton')).toBeTruthy();
    });

    it('hides the run button for js when runnable is explicitly false', () => {
      const el = mount({ lang: 'js', code: 'x', runnable: false });
      expect(shadow(el).querySelector('.runButton')).toBeNull();
    });
    ...
    it('uses a custom highlighter when one is set on the element', () => {
      ...
      const contents = shadow(el).querySelectorAll('.lineContent');
    ...
    it('injects annotation marks into pre-highlighted lines', () => {
      ...
      const html = shadow(el).querySelector('.lineContent')!.innerHTML;
  ...
  describe('annotation interactions', () => {
    const annotated: Partial<CodeGlossConfig> = {
      ...
      code: 'foo bar\nbaz',
      annotations: [
        ann({ id: 'a1', token: 'foo', line: 0, title: 'Foo', text: 'About foo' }),
        ann({ id: 'a2', token: 'baz', line: 1, title: 'Baz', text: 'About baz' }),
      ],
    };

    const queryMark = (el: CodeGlossElement, id: string) =>
      shadow(el).querySelector<HTMLElement>(`mark[data-ann-id="${id}"]`)!;

    it('opens a callout with the annotation content when a mark is clicked', () => {
      const el = mount(annotated);
      queryMark(el, 'a1').click();

      const callout = shadow(el).querySelector('.callout');
      ...
      expect(callout!.querySelector('.calloutBody')!.textContent).toBe('About foo');
    });

    it('dismisses the callout when the same annotation is clicked again', () => {
      const el = mount(annotated);
      queryMark(el, 'a1').click();
      queryMark(el, 'a1').click(); // re-render produces a fresh mark element
      expect(shadow(el).querySelector('.callout')).toBeNull();
    });

    it('switches the active callout when a different annotation is clicked', () => {
      const el = mount(annotated);
      queryMark(el, 'a1').click();
      queryMark(el, 'a2').click();

      const callouts = shadow(el).querySelectorAll('.callout');
      expect(callouts).toHaveLength(1);
      expect(callouts[0].querySelector('.calloutTitle')!.textContent).toBe('Baz');
    });
    ...
    it('ignores keys other than Escape', () => {
      ...
      expect(shadow(el).querySelector('.callout')).toBeTruthy();
    });
    ...
    it('ignores clicks inside the pre that are not on a mark', () => {
      const el = mount(annotated);
      shadow(el)
        .querySelector<HTMLElement>('.lineNumber')!
        .click();
      expect(shadow(el).querySelector('.callout')).toBeNull();
    });
  });
  ...
  describe('connection popover', () => {
    ...
    it('opens the popover with title + body when an interactive arc is clicked', async () => {
      ...
      const popover = shadow(el).querySelector(
        '.connectionTooltip',
      ) as HTMLElement;
    ...
    it('renders only the title when the connection has no body text', async () => {
      ...
      const el = mount({ ...config, connections: [titleOnly] });
      await nextFrame();

      lastDrawArgs().onConnectionClick(titleOnly, new MouseEvent('click'));
      const popover = shadow(el).querySelector('.connectionTooltip')!;
    ...
    it('renders only the body when the connection has no title', async () => {
      ...
      const el = mount({ ...config, connections: [bodyOnly] });
      await nextFrame();
  ...
  describe('copy button', () => {
    it('writes the raw code to the clipboard', () => {
      const el = mount({ lang: 'js', code: 'console.log(1)' });
      shadow(el)
        .querySelector<HTMLButtonElement>('.copyButton')!
        .click();
    ...
    it('swaps the icon to a check mark and reverts after the feedback timeout', () => {
      vi.useFakeTimers();
      const el = mount({ lang: 'js', code: 'x' });
      const btn = shadow(el).querySelector<HTMLButtonElement>('.copyButton')!;
  ...
  describe('run button', () => {
    it('renders captured console output and switches the label to "Run again"', () => {
      const el = mount({ lang: 'js', code: 'console.log("hi")' });
      const btn = shadow(el).querySelector<HTMLButtonElement>('.runButton')!;

      btn.click();

      const output = shadow(el).querySelector('.outputStrip') as HTMLDivElement;
    ...
    it('replaces previous output when run is clicked twice', () => {
      const el = mount({ lang: 'js', code: 'console.log("first")' });
      const btn = shadow(el).querySelector<HTMLButtonElement>('.runButton')!;

      btn.click();
      btn.click();

      const lines = shadow(el).querySelectorAll('.outputLine');
      expect(lines).toHaveLength(1);
  ...
  describe('disconnectedCallback', () => {
    it('cleans up listeners and pending timers without throwing', async () => {
      ...
      vi.useFakeTimers();
      shadow(el).querySelector<HTMLButtonElement>('.copyButton')!.click();
      window.dispatchEvent(new Event('resize'));
  ...
  describe('edge-case branches', () => {
    it('ignores clicks on a mark element with an empty data-ann-id attribute', () => {
      const el = mount({ lang: 'js', code: 'foo' });
      const fakeMark = document.createElement('mark');
      fakeMark.setAttribute('data-ann-id', '');
      shadow(el).querySelector('.pre')!.appendChild(fakeMark);
    ...
    it('exercises the defensive guards when config is unexpectedly null', () => {
      ...
      expect(() => internals.handleRun(document.createElement('button'))).not.toThrow();
