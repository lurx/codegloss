...
**Codegloss stays syntax-agnostic.** Colors come from _your_ highlighter, not from codegloss. When the highlighter reports its chrome (via the `{ background, color }` return) codegloss applies those as `--cg-bg` and `--cg-text` on the host so the block matches the highlighter's theme. No named codegloss theme required.
...
<CodeGloss
	code="..."
	lang="js"
	highlight={codeglossConfig.highlight}
/>;
```
