...
<InstallTabs packages="codegloss @codegloss/react @next/mdx @mdx-js/loader" />
...
Configure the remark plugin in `next.config.ts`. If you've declared a `highlight` in `codegloss.config.ts` (see [Syntax Highlighters](/docs/highlighters)), thread it through so blocks are pre-highlighted at build:
