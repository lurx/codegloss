...
Add `remark-codegloss` to your `docusaurus.config.js`. Use `skipImport: true` because we'll provide the component via Docusaurus's MDX components map (next step):
...
const MDXComponents = require('@theme-original/MDXComponents');
const { CodeGloss } = require('codegloss/react');
