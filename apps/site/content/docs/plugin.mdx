...
- Parses the annotations JSON and forwards its top-level keys (`annotations`, `connections`, `arcs`) as individual props — each key becomes its own prop on the emitted node, not merged
...
Three top-level keys are recognized:

- **`annotations`** — array of [`Annotation`](/docs/api#annotation) objects.
