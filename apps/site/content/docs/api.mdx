...
The Web Component uses a built-in regex highlighter by default (recognizes JS/TS keywords, strings, numbers, comments). For production-grade tokenization, register a real highlighter once at startup — the function runs for every block on the page:
...
`setDefaultHighlighter(fn)` assigns the highlighter on every `<code-gloss>` instance — current and future — and refreshes mounted blocks so the swap is visible without a page reload. Pass `undefined` to revert to the built-in tokenizer.

The `Highlighter` contract is `(code, lang) => string`: return one HTML blob for the whole code and codegloss splits it into lines internally. Anything that emits span-wrapped tokens works (Shiki, Prism, hljs, Starry Night, or your own). See [Syntax Highlighters](/docs/highlighters) for the bundled adapters and the rolling-your-own guide.

> Functions can't live in the JSON config, so this is the dedicated API. Per-instance overrides are still possible via `element.highlight = ...` on a specific `<code-gloss>` reference.
